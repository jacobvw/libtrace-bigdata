        ...
        switch(event.type) {
            ...
            case YAML_SCALAR_EVENT:
                if (strcmp((char *)event.data.scalar.value, "interface") == 0) {
                    ...
                    // the next event should be the interface if not config is incorrect
                ...
                if (strcmp((char *)event.data.scalar.value, "foreach") == 0 ||
                    ...
                    // should now be a a module name if not something is wrong
...
int main(int argc, char *argv[]) {
    ...
    // parse configuration
    bd_conf_t *conf = parse_config(argv[1], global_data);

    libtrace_t *trace = NULL;
    libtrace_callback_set_t *processing = NULL;
    libtrace_callback_set_t *reporter = NULL;

    trace = trace_create(conf->interface);
